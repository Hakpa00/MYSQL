-- INDEX : 목차 , 검색할 때의 범위를 지정 >> 속도 ↑
-- 정렬을 먼저 해줘야함, 카디널리티가 ↑ = 고유값이 높다 = 중복데이터가 별로 없다  

CREATE DATABASE  IF NOT EXISTS `05.04.day08` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `05.04.day08`;
-- MySQL dump 10.13  Distrib 8.0.45, for Win64 (x86_64)
--
-- Host: 127.0.0.1    Database: 4.22
-- ------------------------------------------------------
...
INSERT INTO `customers` VALUES (1,'김민준','010-1234-5678','부산시 해운대구 우동 101','minjun@email.com','2026-04-27 18:57:40'),(2,'이서연','010-2345-6789','부산시 수영구 광안동 202','seoyeon@email.com','2026-04-27 18:57:40'),(3,'박지호','010-3456-7890','부산시 남구 대연동 303',NULL,'2026-04-27 18:57:40'),(4,'최수아','010-4567-8901','부산시 동래구 온천동 404','sua@email.com','2026-04-27 18:57:40'),(5,'정우진','010-5678-9012','부산시 부산진구 전포동 505','woojin@email.com','2026-04-27 18:57:40'),(6,'강하은','010-6789-0123','부산시 연제구 거제동 606',NULL,'2026-04-27 18:57:40'),(7,'조현서','010-7890-1234','부산시 사하구 하단동 707','hyunseo@email.com','2026-04-27 18:57:40'),(8,'윤도윤','010-8901-2345','부산시 북구 구포동 808','doyoon@email.com','2026-04-27 18:57:40'),(9,'임지아','010-9012-3456','부산시 강서구 명지동 909',NULL,'2026-04-27 18:57:40'),(10,'한준서','010-0123-4567','부산시 기장군 일광읍 010','junser@email.com','2026-04-27 18:57:40');
...
-- Dump completed on 2026-04-27 19:19:07
...
where
	customer_id = 3 ;
    -- type ref : index 참조, rows 5개 >> INDEX를 걸었는데 ROW가 너무 많이 나오면 성능 저하 원인
    
-- insert 빈도 수 높은 컬럼 보다 조건으로 사용되는 컬럼을 INDEX 걸어주는게 좋다
-- 복합 INDEX : 순서 중요

SHOW INDEX FROM ORDERS; -- PK 는 무조건 카디널리티가 높음 
DROP INDEX idx_orders_customer ON ORDERS;

-- VIEW (가상테이블)
-- 고객명으로 주문된 모든 정보들을 확인
SELECT
	*
FROM
	ORDERS OD
    LEFT JOIN CUSTOMERS CT ON CT.ID = OD.CUSTOMER_ID
    LEFT JOIN RESTAURANTS RST ON RST.ID = OD.RESTAURANT_ID
    LEFT JOIN MENUS MN ON MN.ID = OD.MENU_ID
WHERE 
	CT.NAME = '최수아';
    
-- 부산시에 속한 가게로 주문된 모든 정보들을 확인
SELECT
	*
FROM
	ORDERS OD
    ...
    LEFT JOIN MENUS MN ON MN.ID = OD.MENU_ID
WHERE 
	RST.ADDRESS LIKE '부산시%';
    
    
-- VIEW 생성
-- * 을 사용하면 테이블에 ID가 여러개 존재해서 나중에 FROM 할때 어떤 ID를 가져와야하는지 혼동 발생 >> ERROR
CREATE VIEW ORDERS_ALL_REF_JOIN_VIEW AS
SELECT 
	OD.id AS OD_ID, 
    OD.Customer_id, 
    OD.restaurant_id, 
    ...
    RST.id AS RST_ID,
    RST.name AS RST_NAME,
    RST.category, 
    RST.address AS RST_ADDRESS,
    RST.rating, 
    
    MN.id AS MN_ID,
    MN.restaurant_id AS MN_RESTAURANT_ID, 
    MN.name AS MN_NAME, 
    MN.price
FROM
	ORDERS OD
...
SELECT
	*
FROM
	ORDERS_ALL_REF_JOIN_VIEW;

-- 메뉴테이블과 가게 테이블을 조인한 단순 SELECT 문을 MENUS_VIEW로 저장한 후 가게별 최고가 메뉴를 조회
SELECT
	*
FROM
	MENUS MN
...
FROM (
	...
	GROUP BY
		...
		RST_NAME,
		MN_NAME,
		PRICE) AS SUB_MENUS_VIEW
WHERE
	RANK_NUMBER = 1;
    
-- [TRANSACTION] : ALL OR NOTHING
-- A:원자성 - 모두 성공 / 모두 실패
-- C:일관성 - 트랙잭션 전후 데이터 무결성 유지 
-- I:격리성 - 동시 트랜잭션이 서로 간섭 X 
-- D:지속성 - 에러가 나도 데이터 유지 

SELECT * FROM CUSTOMERS;
INSERT INTO CUSTOMERS
VALUES (DEFAULT,'김준일','010-1234-5678','부산시 동래구 사직동 123','SKJIL1218@GMAIL.COM',DEFAULT); 

DELETE FROM CUSTOMERS WHERE ID = 11;
COMMIT; -- 물리적 데이터 적용(저장)
...
SAVEPOINT P1; -- rollback이랑 똑같음
INSERT INTO RESTAURANTS
VALUES (DEFAULT, '코리아치킨집', '치킨', '부산시 부산진구 에이원프라자', 4.0);

SELECT MAX(ID) FROM restaurants;

SAVEPOINT P2;
INSERT INTO menus
SELECT NULL, MAX(ID),'후라이드 치킨3',20000 FROM restaurants;
