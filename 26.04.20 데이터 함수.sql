...
-- SELECT(데이터를 표에 표현) 컬럼명 FROM 테이블 WHERE 조건 ORDER BY LIMIT
-- AS 알리아스 
...
SELECT concat(NAME,'[',CATEGORY,']') AS 가게정보 FROM restaurants;

-- SUBSTRING 문자열 자르기
SELECT substring(NAME,1,3) FROM RESTAURANTS;
...
-- 문자열 찾아서 바꾸기 - CTRL H 
SELECT replace('  해 운 대  ',' ','-');

-- 문자열 길이 
-- LENGTH : 바이트 수 - 바이트:2^8=256  
-- CHAR_LENGTH : 실제 글자 수 
SELECT * FROM restaurants WHERE char_length(REPLACE(NAME,' ','')) = 5; -- 띄어쓰기 빼고 진짜 글자수가 5개 인 애들 
...
SELECT ceil(1234.11);
SELECT FLOOR(1234.999);
SELECT TRUNCATE(1234.999999,2);
...
-- 날짜 함수
SELECT now();
SELECT CURDATE(); -- 현재 컴퓨터 서버의 날짜,시간
SELECT CURTIME();

SELECT DATE_FORMAT(CURDATE(),'%Y년 %m월 %d일');
SELECT DATE_FORMAT(CURDATE(),'%y년 %m월 %d일');
SELECT DATE_FORMAT(curtime(),'%H시 %i분 %s초');
SELECT DATE_FORMAT(now(),'%Y년 %m월 %d일 %H시 %i분 %s초 %W요일 %w');
select concat(elt(dayofweek(curdate()),'일','월','화','수','목','금','토'),'요일'); -- 1,2,3,4,5,6,0
SELECT year(curdate());
...
SELECT date_add(CURDATE(), INTERVAL 7 DAY); -- 유효기간,유지보수기간
...
SELECT date_sub('2026-01-01',INTERVAL 6 MONTH);
SELECT date_ADD('2026-01-01',INTERVAL 6 MONTH);

SELECT date_sub('2026-01-01',INTERVAL 6 MONTH) <= '2026-04-20'
	AND date_add('2026-01-01',INTERVAL 6 MONTH) >= '2026-04-20';

-- 조건 IF,IFNULL,CASE WHEN
INSERT INTO RANKS 
	VALUES
		(DEFAULT,NULL,'10020'),
		(DEFAULT,'F',NULL),
...
SELECT
	ID,
    ifnull(`NAME`,'이름없음'), -- 데이터 바꾸는게 아님 이때 볼때만 
    if(`RANK` IS NULL,'랭킹없음','RANK'),
    NULL, -- NULL 넣고
    IFNULL(NULL,NOW()),
    IF(MOD(ID,2)=0,CURDATE(),CURTIME())
FROM 
	RANKS;
...
SELECT
    ...
    RATING,
    CASE
		WHEN RATING >= 4.5 THEN '별5개'
		WHEN RATING >= 4.0 THEN '별4개'
		WHEN RATING >= 3.5 THEN '별3개'
		WHEN RATING >= 3.0 THEN '별2개'
        ELSE '별1개'
    END AS STAR
FROM
	restaurants;
    
-- DISTINCT 중복 제거 AS 알리아스
SELECT DISTINCT
	CATEGORY,
    RATING
FROM
